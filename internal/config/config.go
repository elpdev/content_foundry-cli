...
type Config struct {
	...
	DefaultBrandID   int64  `toml:"default_brand_id"`
	DefaultBrandSlug string `toml:"default_brand_slug"`
}
...
func Dir() string {
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "content_foundry")
	}
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "content_foundry")
}
